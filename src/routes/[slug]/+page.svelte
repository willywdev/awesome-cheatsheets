<script>
import CardContainer from '../../lib/primitives/CardContainer.svelte';
export let data;
...
<svelte:head>
	<title>AC - {slug.toUpperCase()}</title>
...
<CardContainer>
		{#each cheatsheet as item}
			<a href={item.url} target="_blank" class="block max-w-sm p-4 border rounded-lg shadow border-surface-300 bg-surface-100 hover:bg-gray-100 dark:bg-surface-700 dark:border-surface-500 dark:hover:bg-surface-500">
				<li>
